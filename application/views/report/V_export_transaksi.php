...
<h6 style="font-weight: bold; margin: 5px;"><?= $tgl_awal ?> s/d <?= $tgl_akhir ?></h6>
...
<?php $no=1; foreach ($trn as $d): ?>
  ...
  <table border="0">
        ...
        <tr>
            <td>Pelanggan</td>
            <td>: <?= ($d['pelanggan']) ? $d['pelanggan'] : 'Umum' ?></td>
        </tr>
  </table>
  <br>
      ...
      <tbody>
        ...
        foreach ($dt as $t): ?>
          <tr>
            ...
            <td align="right">Rp. <?= number_format($t['discount'], 0,'.','.') ?></td>
            <td align="right">Rp. <?= number_format($t['subtotal'], 0,'.','.') ?></td>
          </tr>
        <?php $i++; $tot_dis += $t['discount']; endforeach; ?>
      </tbody>
  </table>
  <br>
  <table border="0">
    ...
    <tr>
        <td>Total Bayar</td>
        <td>: Rp. <?= number_format($d['tunai'], 0,'.','.') ?></td>
    </tr>
  </table>
  <br><hr>
<?php $no++; endforeach; ?>
